...
router.post('/getcartorderdetailitems', (req, res) => {
    ...
    let qr = `SELECT resturant.charges,customer.customer_Id,item.*,cart.total as "totalp",cart.cashondelivery,cart.paywithcard,orderitem.Price as "OPrice",orderitem.Quantity ,orderitem.id as "orderitemid",customer.name,cart.Status,cart.DateTime,cart.cart_Id FROM orderitem 
    inner join cart on cart.cart_Id=orderitem.Order_ID 
    inner join customer on customer.customer_Id=cart.customer_Id 
    INNER join item on item.ID = orderitem.ProductID 
    INNER join resturant on orderitem.resturant_ID = resturant.ID 
    WHERE cart.cart_Id=${customer_Id}`
    dbconfig.query(qr, (err, result) => {
        if (!err) {
            res.json({
                data: result
